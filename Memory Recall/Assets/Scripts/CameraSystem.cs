...
public class CameraSystem : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (player.transform.position.y > updatePos + 0.1f)
            {
                vcam.m_Lens.OrthographicSize = Mathf.Lerp(vcam.m_Lens.OrthographicSize, vcam.m_Lens.OrthographicSize + 20f * Mathf.Abs(player.transform.position.y - updatePos), Time.deltaTime);
                updatePos = player.transform.position.y;
            }
            else if (player.transform.position.y < updatePos - 0.3f)
            {
                vcam.m_Lens.OrthographicSize = Mathf.Lerp(vcam.m_Lens.OrthographicSize, vcam.m_Lens.OrthographicSize - 15f * Mathf.Abs(player.transform.position.y - updatePos), Time.deltaTime);
                updatePos = player.transform.position.y;
            }
